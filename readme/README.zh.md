...
| 9 | 全程用贵模型烧额度，弱模型又做错，手动切换烦 | 按难度自动路由：micro 直做 / 普通 flash / 复杂与复核 pro + fallback |
| 10 | 实际跑哪个模型对不上账 | recommended 与 actual 分开记录，来源如实标注，未知记 unknown |
...
| 15 | 选模型靠感觉，没有真实数据 | 每次执行记录实际模型与任务完成 → 月度仪表盘；未采集的失败/耗时如实显示「未记录」，绝不虚构 |
...
- **开源仓库**：https://github.com/kakadeka/Baton（仅公开四件套 + DSH 组合包，经官方发布流程同步，不含任何项目隐私与凭据）
