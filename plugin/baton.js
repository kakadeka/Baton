// Baton 插件 v3.1.0-P5（Host half）—— 纯 JavaScript，无 import/require。敏感词清单外置本机配置，代码不硬编码。
// 架构：本地操作全机械（会话策略解锁写入）；网络 git（fetch/push/ls-remote）由主会话 pwsh 执行；
// 插件 baton_verify_push 对远端 SHA 与本地 HEAD 做纯只读机械核验（绝不写文件、绝不提交）。
return {
  name: 'baton',
  // P5 关键修复：shell/fs 必须进 inject（硬依赖）。组合里行是并行挂载的，
  // 若只在 apply 里 ctx.get('shell')，apply 可能在 shell/fs 行挂完之前执行，
  // 守卫提前 return → 16 个工具全部静默缺失且永不重试（npm 组合包首版问题）。
  // 双保险：inject 保证顺序 + services() 惰性重读兜底（动态沙箱/特殊组合）。
  inject: ['tools', 'shell', 'fs'],
  apply(ctx) {
